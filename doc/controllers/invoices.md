...
| `dateField` | [`?string(InvoiceDateField)`](../../doc/models/invoice-date-field.md) | Query, Optional | The type of filter you would like to apply to your search. Use in query `date_field=issue_date`.<br><br>**Default**: `InvoiceDateField::DUE_DATE` |
...
```php
function updateInvoice(int $subscriptionId, string $uid, ?UpdateInvoiceRequest $body = null): InvoiceResponse
```
...
```php
function deleteInvoice(int $subscriptionId, string $uid): void
```
