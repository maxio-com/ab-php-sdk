...
|  --- | --- | --- | --- |
| `subscriptionId` | `int` | Template, Required | The Chargify id of the subscription |
| `calendarBillingResumptionCharge` | [`?string(ResumptionCharge)`](../../doc/models/resumption-charge.md) | Query, Optional | (For calendar billing subscriptions only) The way that the resumed subscription's charge should be handled |
...
```php
function pauseSubscription(int $subscriptionId, ?PauseRequest $body = null): SubscriptionResponse
```
...
$body = PauseRequestBuilder::init()
    ->hold(
        AutoResumeBuilder::init()
            ->automaticallyResumeAt(DateTimeHelper::fromRfc3339DateTime('2017-05-25T11:25:00Z'))
            ->build()
    )
    ->build();
...
$body = PauseRequestBuilder::init()
    ->hold(
        AutoResumeBuilder::init()
            ->automaticallyResumeAt(DateTimeHelper::fromRfc3339DateTime('2019-01-20T00:00:00'))
            ->build()
    )
    ->build();
