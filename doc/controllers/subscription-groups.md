...
```php
function readSubscriptionGroup(string $uid, ?array $mInclude = null): FullSubscriptionGroupResponse
```
...
| `mInclude` | [`?(string(SubscriptionGroupInclude)[])`](../../doc/models/subscription-group-include.md) | Query, Optional | Allows including additional data in the response. Use in query: `include[]=current_billing_amount_in_cents`. |
...
```php
function findSubscriptionGroup(string $subscriptionId): FullSubscriptionGroupResponse
```
...
```php
function addSubscriptionToGroup(
    int $subscriptionId,
