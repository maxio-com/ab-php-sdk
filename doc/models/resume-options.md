...
|  --- | --- | --- | --- | --- | --- |
| `requireResume` | `?bool` | Optional | Chargify will only attempt to resume the subscription's billing period. If not resumable, the subscription will be left in its current state. | getRequireResume(): ?bool | setRequireResume(?bool requireResume): void |
...
```php
use AdvancedBillingLib\Models\Builders\ResumeOptionsBuilder;

$resumeOptions = ResumeOptionsBuilder::init()
    ->requireResume(false)
