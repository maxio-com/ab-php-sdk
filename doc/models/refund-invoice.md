...
| `voidInvoice` | `?bool` | Optional | If `apply_credit` is set to false and refunding full amount, if `void_invoice` is set to true, invoice will be voided after refund. Defaults to `false`. | getVoidInvoice(): ?bool | setVoidInvoice(?bool voidInvoice): void |
...
$refundInvoice = RefundInvoiceBuilder::init(
    'amount2',
    'memo4',
    36
)
