...
| `interval` | `?int` | Optional | The numerical interval. e.g., an interval of ‘30’ coupled with an interval_unit of day would mean this product price point would renew every 30 days. | getInterval(): ?int | setInterval(?int interval): void |
...
$productPricePoint = ProductPricePointBuilder::init()
    ->id(10)
    ->name('name0')
    ->handle('handle6')
    ->priceInCents(178)
