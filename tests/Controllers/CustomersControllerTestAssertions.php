...
final class CustomersControllerTestAssertions
{
    ...
    ): void
    {
        ...
        // Remove maxioid because it's auto-generated by the API
        unset($expectedCustomerJson['maxioid']);
        unset($customerJson['maxioid']);

        $this->testCase::assertEquals($expectedCustomerJson, $customerJson);

        // Assert maxioid separately — server-generated, unique per customer
        $maxioid = $customer->jsonSerialize()['maxioid'] ?? null;
        $this->testCase::assertNotNull($maxioid);
        $this->testCase::assertMatchesRegularExpression('/^cus_[a-z0-9-]+$/', $maxioid);
    }
