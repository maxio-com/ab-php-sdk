...
use AdvancedBillingLib\Exceptions\TooManyManagementLinkRequestsErrorException;
...
class BillingPortalController extends BaseController
{
     ...
     *
     * This documentation is focused on how to configure the Billing Portal Settings, as well as Subscriber
     * Interaction and Merchant Management of the Billing Portal.
     *
        ...
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/portal/customers/{customer_id}/enable.json')
            ...
            ->parameters(
                ...
                QueryParam::init('auto_invite', $autoInvite)
                    ->commaSeparated()
                    ->serializeBy([AutoInvite::class, 'checkValue'])
            );
     ...
     * @throws ApiException Thrown if API call fails
     */
    public function readBillingPortalLink(int $customerId): PortalManagementLink
    {
     ...
     * @throws ApiException Thrown if API call fails
     */
    public function resendBillingPortalInvitation(int $customerId): ResentInvitation
    {
        $_reqBuilder = $this->requestBuilder(
            RequestMethod::POST,
