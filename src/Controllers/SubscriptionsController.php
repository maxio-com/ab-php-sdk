...
class SubscriptionsController extends BaseController
{
     ...
     * topics:
     *
     * + [Subscriptions Reference](https://maxio-chargify.zendesk.com/hc/en-us/articles/5405577172749-
     * Subscription-Introduction)
     * + [Subscriptions Actions](https://maxio-chargify.zendesk.com/hc/en-us/articles/5405510556557-
     * Actions)
     * + [Subscription Cancellation](https://maxio-chargify.zendesk.com/hc/en-us/articles/5405510556557-
     * Actions#initiate-cancellation)
     * + [Subscription Reactivation](https://maxio-chargify.zendesk.com/hc/en-us/articles/5404559291021-
     ...
     * //maxio-chargify.zendesk.com/hc/en-us/articles/5405246782221#payment-method-settings)).
     *
     ...
     *
     * An existing customer may be specified by a `customer_id` (ID within Chargify) or a
     * `customer_reference` (unique value within your app that you have shared with Chargify via the
     * reference attribute on a customer). You may also pass in an existing payment profile for that
     ...
     *
     * If your intent is to charge your subscribers tax via [Avalara Taxes](https://maxio-chargify.zendesk.
     * com/hc/en-us/articles/5405275711885-Avalara-VAT-Tax) or [Custom Taxes](https://maxio-chargify.
     * zendesk.com/hc/en-us/articles/5405069041549-Custom-Taxes), there are a few considerations to be made
     * regarding collecting subscription data.
     ...
     *
     * + A subscription to a [taxable product](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405246782221-Product-Editing#tax-settings)
     * + [Full valid billing or shipping address](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405356114317#full-address-required-for-taxable-subscriptions) to identify the tax
     * locale
     * + The portion of the address that houses the [state information](https://maxio-chargify.zendesk.
     * com/hc/en-us/articles/5405356114317#required-state-format-for-taxable-subscriptions) of either
     * adddress must adhere to the ISO standard of a 2-3 character limit/format.
     * + The portion of the address that houses the [country information](https://maxio-chargify.zendesk.
     * com/hc/en-us/articles/5405356114317#required-country-format-for-taxable-subscriptions) must adhere
     * to the ISO standard of a 2 character limit/format.
     *
     ...
     * Chargify. Please be aware that <b>collecting and sending Chargify raw card details requires PCI
     * compliance on your end</b>; these examples are provided as guidance. If your business is not PCI
     * compliant, we recommend using Chargify.js to collect credit cards or bank accounts.
     *
     ...
     * allow the card to be displayed properly in the Chargify UI.
     *
     * ```json
     * {
     * "subscription": {
     ...
     * + [Full documentation on GoCardless](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5404501889677)
     *
     ...
     * + [Full documentation on Stripe Direct Debit](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405050826765-Stripe-SEPA-and-BECS-Direct-Debit)
     *
     ...
     * For more information on Stripe Direct Debit, please view the following two resources:
     *
     * + [Payment Profiles via API for Stripe BECS Direct Debit](https://developers.chargify.com/docs/api-
     * docs/1f10a4f170405-create-payment-profile#stripe-becs-direct-debit)
     * + [Full documentation on Stripe Direct Debit](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405050826765-Stripe-SEPA-and-BECS-Direct-Debit)
     *
     * + [Using Chargify.js with Stripe SEPA or BECS Direct Debit - minimal example](https://developers.
     ...
     * com/docs/developer-docs/ZG9jOjE0NjAzNDIy-examples#full-example-with-becs-direct-debit-stripe-
     * gateway)
     *
     *
     * ```json
     ...
     * is referred to in our help docs as a [post-authentication flow](https://maxio-chargify.zendesk.
     * com/hc/en-us/articles/5405177432077#psd2-flows-pre-authentication-and-post-authentication). The
     * server returns `422 Unprocessable Entity` in this case with the following response:
     *
     * ```json
     * {
     * "errors": [
     ...
     * attributes.
     *
     * Full documentation on how import Subscriptions using the **import tool** in the Chargify UI can be
     * located [here](https://maxio-chargify.zendesk.com/hc/en-us/articles/5404863655821#imports-0-0).
     *
     ...
     * Import](https://maxio-chargify.zendesk.com/hc/en-us/articles/5404863655821#important-notices-and-
     * disclaimers) instructions to understand the repurcussions of a large import.
     *
     ...
     * create the payment profile in Chargify - setting verified=true, then create a subscription using the
     ...
     *
     * For sites making use of the [Relationship Billing](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405078794253-Introduction-to-Invoices) and [Customer Hierarchy](https://maxio-chargify.
     * zendesk.com/hc/en-us/articles/5404900384141) features, it is possible to create subscriptions within
     * a customer hierarchy.  This can be achieved through the API by passing group parameters in the
     ...
     *
     * For sites making use of [Relationship Billing](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5405078794253-Introduction-to-Invoices) it may be desireable to create a subscription as
     * part of a [subscription group](https://maxio-chargify.zendesk.com/hc/en-us/articles/5405577356173)
     * in order to rely on [invoice consolidation](https://maxio-chargify.zendesk.com/hc/en-
     * us/articles/5404980119949). This can be achieved through the API by passing group parameters in the
     * Create Subscription request.  The `group` parameters are optional and consist of the required
